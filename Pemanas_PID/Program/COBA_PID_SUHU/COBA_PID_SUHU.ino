...
void setup() {
  ...
  stepper.setCurrentPosition(posisiAwal);
...
void bacaPID(int x, int y) {
  ...
  prevPID_total = PID_total;

  if (y < Temp && Off_Step == true) {
    PID_State = false;
    stepPosisi(0);
    posisiSaatIni = 0;
  ...
  if (PID_State == true) {
    if (PID_total > 100) {
     ...
     else if (PID_total > 5 && PID_total < 75) {
      PID_total_Step = -580;
      stepPosisi(PID_total_Step);
    } else if (PID_total > -100 && PID_total < 0) {
      PID_total_Step = 0;
      stepPosisi(PID_total_Step);
    }
    else {
    PID_total = map(PID_total, 0, 400, 0, -400);
    }
  }
  prev_Error = Error;
}
